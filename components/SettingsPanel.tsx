...
interface Props {
  ...
  disabled?: boolean;
  sizeMin: number;
  setSizeMin: (n: number) => void;
  sizeMax: number;
  setSizeMax: (n: number) => void;
  speedMul: number;
  setSpeedMul: (n: number) => void;
  rotMul: number;
  setRotMul: (n: number) => void;
  splitDeflect: number;
  setSplitDeflect: (n: number) => void;
  bounceDeflect: number;
  setBounceDeflect: (n: number) => void;
  cubeAlpha: number;
  setCubeAlpha: (n: number) => void;
  glitchSec: number;
  setGlitchSec: (n: number) => void;
  onReset: () => void;
}
      ...
      <div className="panel__body panel-collapsible expanded" id="panelBody" ref={bodyRef}>
        <fieldset disabled={isDisabled} style={{ border: "none", padding: 0, margin: 0 }}>
          ...
          <div className="panel__actions">
            <button id="btnReset" disabled={!ok || isDisabled} onClick={p.onReset}>
              {isDisabled ? "글리치 중..." : "초기화/재시작"}
            </button>
          </div>
        </fieldset>
      </div>
